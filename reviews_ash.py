...
restaurants = pd.read_csv("../yelp-dataset/merged_NV_restaurant.csv", encoding='utf-8')
restaurants = restaurants.dropna(axis=1, thresh=300)
#restaurant_reviews = pd.read_csv("../data/yelp-dataset/yelp_review.csv")

#df_merged = pd.merge(restaurants, restaurant_reviews, on='business_id')

df_merged = pd.read_csv('../yelp-dataset/review_shortened_rest_NV.csv', encoding='utf-8')
...
#X = cv.fit_transform(cleanedList).toarray()
